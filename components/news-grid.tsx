            ...
            return (
              <article key={article.id} className={articleClasses}>
                <div className="relative overflow-hidden rounded-lg mb-4">
                  <img src={article.thumbnail || "/placeholder.svg"} alt={article.title} className={imageClasses} />
                    {article.section}
                </div>
                <div className="space-y-2">
                  <h3 className={titleClasses}>{article.title}</h3>
                  <p className="text-muted-foreground leading-relaxed text-pretty">{article.description}</p>
